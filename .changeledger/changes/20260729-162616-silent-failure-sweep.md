...
- **2026-07-29T18:07:29Z** `[note]` Mandato del review, registrado antes de delegar: superficie que gobierna — el diff del commit 1a639c77 (5 módulos de src + 5 suites), los nueve criterios con re-derivación de mutantes y los rojos reproducidos. Puntos de escrutinio: la enmienda de CR2 del orquestador contra el árbol, el narrowing doble de CR7 (incluido el caso no nombrado archived-bajo-abierto), el split del test preexistente 105456 CR9, y la decisión case-insensitive incondicional de CR8. Revisor top-tier: 450 líneas sobre guards de seguridad del commit y semántica de check.
- **2026-07-29T18:24:04Z** `[review]` in-review → in-progress (retry): Finding 1 (medium-high, confirmado ejecutándolo): el lowercase del set expected abre un bypass nuevo fail-open — en filesystem case-sensitive el gemelo mal-caseado de un documento declarado se commitea donde el código viejo lo rechazaba. El fix es estrictamente más fuerte que ambas eras: lowercase solo en la comparación de prefijo (fail-closed), expected.has(file) exacto. Findings 2-4 informativos, follow-ups registrados: tdd= en agent-context sin definición servida (misma clase que CR5, superficie distinta), consecuencia consumer-visible del narrowing (b) de CR7, y CR4 anulado bajo tdd:false.
