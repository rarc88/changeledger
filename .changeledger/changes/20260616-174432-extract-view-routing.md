...
## Plan
- [x] Move `createRequestListener`, request dispatch, and `staticFile` from `src/commands/view.mjs` into a `src/viewer/server/` router module covered by `test/view.test.mjs`.
  - **Resolved:** `2026-06-16T20:57:39Z`
- [x] Move `isLocalHost`, `isAuthorizedWrite`, and hostname parsing into a `src/viewer/server/` security module covered by the existing localhost/write-authorization tests.
  - **Resolved:** `2026-06-16T20:57:42Z`
- [x] Leave `src/commands/view.mjs` responsible for CLI argument parsing, token creation, server startup, and browser opening only.
  - **Resolved:** `2026-06-16T20:57:45Z`
- [x] Preserve the public exports needed by existing tests, either by re-exporting from `view.mjs` or updating tests to import the new modules directly.
  - **Resolved:** `2026-06-16T20:57:48Z`

## Log
- **2026-06-16T20:47:03Z** `[status]` draft → approved
- **2026-06-16T20:55:53Z** `[status]` approved → in-progress
- **2026-06-16T20:55:53Z** `[owner]` set: Roberto Ruiz (auto)
- **2026-06-16T20:57:51Z** `[status]` in-progress → in-review
- **2026-06-16T21:00:02Z** `[review]` in-review → in-progress (retry): view command still owns domain logic and router imports back from command module
- **2026-06-16T21:01:22Z** `[status]` in-progress → in-review
- **2026-06-16T21:02:41Z** `[review]` in-review → done (delegated subagent, clean context)
