...
- [ ] Añadir a `assertTransition` en `src/lifecycle.mjs` el rechazo de `in-progress → in-review` cuando `reviewRequired` no es `true`; verify: `node --test test/lifecycle.test.mjs` (CR1, CR3, CR4)
- [ ] Cubrir en `src/commands/agent.mjs` que el rechazo deja el fichero byte a byte intacto y el `## Log` sin evento nuevo; verify: `node --test test/agent.test.mjs` (CR2)
- [ ] Cubrir en `src/commands/agent-context.mjs` y `src/commands/context.mjs` que el modo revisión es inalcanzable para un tipo sin `review_required`; verify: `node --test test/agent-context.test.mjs` (CR5, CR6)
