...
- [ ] Crear `src/commands/commit.mjs` con resolución de change activo y composición del subject; verify: `node --test test/commit.test.mjs` (CR1, CR2, CR3, CR4)
- [ ] Registrar el subcomando en `bin/changeledger.mjs` con help y opciones; verify: `pnpm test` (CR1)
- [ ] Añadir `--commits` a `src/commands/check.mjs` con parseo del rango sobre `src/git.mjs`; verify: `node --test test/check.test.mjs` (CR5, CR6)
- [ ] Actualizar `templates/contract/implement.md` con el formato canónico y las exenciones; verify: `pnpm test` (CR7)
