...
- **And** `review` contiene `Review-required work must be checked by a fresh subagent with clean context`: la regla 6 antigua salió de core clasificada como movida y ningún test la afirmaba, la misma clase de defecto que el review señaló en las demás movidas
...
- **2026-07-27T15:41:29Z** `[note]` Tarea 8: el comentario de clasificacion del pin de core.md pasa a ser verdadero regla a regla, comprobado contra el fichero dueño. (a) Las movidas dejan de afirmar literalidad: solo close.md la tiene ('changeledger graduate <id> --skip [reason]'); implement.md dice 'After approved -> in-progress, create a baseline commit of the approved change document before code', review.md dice 'Review-required work must be checked by a fresh subagent with clean context and a model sized to the review difficulty'. (b) La entrada de compartir el codebase, que en su propio texto decia 'preserved in delegation.md', queda reclasificada MOVIDA con el texto real 'Tell coding delegates they share the codebase: ... do not revert others' edits and report overlapping changes instead of silently resolving them'. (c) 'Files are the source of truth and may be edited directly' y 'CLI helpers are optional and preferred for error-prone operations' pasan de RETIRADAS a MOVIDAS a spec.md, seccion 'Repository layout and creation': 'Files remain the source of truth and may be edited directly, but prefer the CLI for timestamps, enums and markers that are easy to mistype'. (d) Recuento restated: 7 REEMPLAZADAS, 3 RESTITUIDAS, 3 RETIRADAS y 3 entradas MOVIDAS que cubren 7 reglas. (e) Reglas antes sin clasificar, ahora clasificadas: 'work performed without the CLI may diverge' RETIRADA del contrato -solo sobrevive en README.md como narrativa de producto-, la prohibicion inverificable 'Never request a preview, summary or voluntary line, byte or token cap' RETIRADA como entrada propia, la cola de la regla 3 'An approved change governs code in scope' MOVIDA a implement.md, y las tres reglas conservadas con reformulacion (divergencia previa, regla 5 sin 'while working', regla 4 'One change at a time'). El bloque PRESERVED deja de decir 'verbatim' de los ocho estados: siete lo son y el de in-validation queda extendido por el stop restituido. (f) Pin para la regla 6 antigua: DENTRO de la superficie de este change, porque CR11 exige que cada regla que sale de core siga afirmada en su dueño y la reclasificacion de (a) seria inverificable sin el. Añadido a '124835 CR11' sobre el contexto review compuesto y verificado con mutante aislado -quitar 'fresh' de review.md-: muere.
- **2026-07-27T15:43:35Z** `[note]` Tarea 9: 'pnpm verify' verde con el change todavia en in-progress. lint (biome, 82 ficheros sin arreglos), 809 tests con 0 fallos y 'changeledger check'. Ultima linea exacta: 'OK 18 change(s) valid - 203 not validated (archived or discarded)', exit 0. Medida final del core compuesto: 172 lineas emitidas / 173 en la convencion del test (output.split('\n').length) / 9782 bytes, contra el target estricto 175 lineas / 11000 bytes: quedan 2 lineas y 1218 bytes de margen. core.md = 166 lineas / 9550 bytes; el framing sigue aportando +6 lineas emitidas / +232 bytes. Ni budgets.yml ni ningun fichero bajo src/ se ha tocado; los unicos ficheros del change son templates/contract/core.md, test/context.test.mjs, test/cli.test.mjs y este documento. La transicion a in-review la despacha el orquestador.
- **2026-07-27T15:46:44Z** `[note]` Mandato de review de esta ronda: COMPLETA sobre el comentario de clasificacion y el barrido de reglas huerfanas, SPOT CHECK sobre el resto del delta. Justificacion: ese comentario ya resulto falso en cuatro entradas la ronda anterior y es el unico artefacto que asegura que ninguna regla se pierda; el resto de la correccion es acotado y ya trae mutantes. Nota de proceso: el orquestador instruyo commit por tarea del Plan, lo que contradice la regla de aislamiento de correccion de implement.md; el implementador la senalo en vez de obedecer en silencio. Se trata como alcance enmendado por cuatro tareas nuevas autorizadas por el humano, no como correccion desnuda.
