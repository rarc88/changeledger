...
- **2026-07-23T20:25:39Z** `[validation]` in-validation → in-progress (agent rejected): Auditoría externa: el CR literal pasa pero authority.yml AUSENTE con refs v2 presentes (refs/changeledger/confirmed) cae en silencio a modo worktree — downgrade por borrado/rama pre-cutover. Contención en este alcance: authority ausente + refs v2 → fail closed con error accionable. La resolución de autoridad independiente del checkout va en change arquitectónico separado.
- **2026-07-23T20:31:29Z** `[note]` Contención implementada (sin commit hasta confirmación humana): presentReplicaRef sondea confirmed/pending/observed; authority ausente + ref v2 presente falla cerrado con error accionable (ref exacto, ruta del archivo, restauración via rama post-cutover o state activate); legacy sin refs conserva modo worktree. 47/47 en test/ledger-store.test.mjs, rojo-verde verificado. La resolución independiente del checkout queda en el draft 20260723-202646.
