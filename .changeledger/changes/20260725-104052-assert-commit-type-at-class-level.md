...
- **And** `state doctor --online` sobre esa misma adopción reporta `ok` sin
  problemas, y el valor observado de la fuente es el commit pelado: todo lector
  del valor registrado tiene que pelar igual que quien lo registra
- **And** si esa fuente desaparece del remoto y existe otra ref cuyo nombre la
  tiene como prefijo (`refs/tags/v10` frente a `refs/tags/v1`), el doctor online
  reporta `source advanced or disappeared: origin:refs/tags/v1` con el valor
  observado en `null`, sin confundirla con la hermana y sin fallar por formato:
  la ausencia se decide por el nombre exacto, no por si el glob casó algo
...
### CR10 — La reparación explícita sigue siendo posible

- **Given** un repo con réplica activada cuyo `refs/changeledger/pending` apunta a
  un tag anotado, es decir la corrupción que toda lectura rechaza
- **When** se ejecuta `state abort --pending --offline`
- **Then** la operación tiene éxito, borra la ref y no lanza
- **And** las lecturas vuelven a funcionar sin intervención manual sobre `.git`

  Clasificar también esta ruta dejaría inalcanzable la única salida que el
  `README.md` documenta, justo en el caso en que se necesita, con `git update-ref`
  a pelo como único remedio. Descartar una ref no adopta nada, así que se permite
  sobre el estado que el resto rechaza.
...
- [x] Reconciliar el comentario de `assertCommitTip` en `src/state-store.mjs`, que hoy afirma paridad con una clasificación que el resolvedor de lecturas no tenía; verify: `node --test test/state-store.test.mjs` (support)
  - **Resolved:** `2026-07-25T11:25:47Z`
- [x] Clasificar en el resolvedor compartido `readStateReplica` de `src/state-store.mjs` las tres refs de la réplica, y el `state_ref` de una autoridad v1 en `src/ledger-store.mjs`, con tests que cubran carga, `state status`, recovery y v1; verify: `node --test test/ledger-store.test.mjs test/state-store.test.mjs` (CR9)
  - **Resolved:** `2026-07-25T13:25:07Z`
...
- [x] Pelar en `remoteCommit` de `src/state-migration.mjs` el valor que el doctor online compara, restaurar en `abortStatePending` de `src/state-store.mjs` la ruta offline sin clasificar y documentar el remedio en `README.md`, con tests para el doctor con fuente-tag y para el abort sobre un pending corrupto; verify: `node --test test/state-migration.test.mjs test/ledger-store.test.mjs` (CR1, CR10)
  - **Resolved:** `2026-07-25T13:25:07Z`
- [x] Anclar CR8 con el ciclo completo en ambos formatos de objeto y verificar por mutación sobre `src/state-migration.mjs`, `src/ledger-store.mjs` y `src/state-store.mjs` que retirar cualquiera de las clasificaciones rompe exactamente su propio test, con el control corrido sobre la suite completa y no sobre un subconjunto; verify: `pnpm verify` y `node --test test/state-migration.test.mjs` (CR8)
...
- **2026-07-25T13:11:10Z** `[review]` in-review → in-progress (retry): Sin séptimo sitio: la clase queda cerrada y el revisor re-derivó el inventario por su cuenta. Pero F1 bloqueante: mi propio fix de CR1 introdujo una nueva pelada de un solo lado. El chequeo de deriva online del doctor en state-migration 1602 lee el OID crudo de ls-remote y lo compara contra source.commit, que ahora está pelado, así que state doctor --online reporta 'source advanced or disappeared' sobre un repo sano cuya fuente es un tag anotado, mientras su hermano offline en 1572 sí pela. Es la misma asimetría que este change existe para cerrar, reintroducida una capa más arriba y en una ruta que el propio change abrió, porque antes un source de solo-tag no se podía ni preparar. F2 bloqueante: el aserto de 'ninguna rama de recovery' en los tests de CR9 y CR5 usa el patrón for-each-ref 'refs/heads/changeledger/recover-' sin glob, que nunca casa; verificado que una rama recover-<oid> materializada pasa desapercibida, así que la cláusula añadida para anclar la regresión de la ronda 2 es teatro. El matcher del diagnóstico sí ancla. F3 usabilidad: abort --pending --offline, documentado en README 147 como la salida explícita, quedó inalcanzable justo para el caso en que se necesita, porque abortStatePending lee a través de readStateReplica; no hay subcomando que resetee una ref de réplica y el único remedio es git update-ref a pelo, sin decirlo en ningún sitio. F4: la tarea 5 del Plan sigue sin resolver. F5: la tarea 7 dice cinco clasificaciones y son siete.
- **2026-07-25T13:36:42Z** `[note]` Corrección del tercer retry, con certificación propia antes de entregar en vez de dejársela al revisor. La ronda 3 confirmó que no hay séptimo sitio y que la clase queda cerrada; sus dos bloqueantes eran míos, no cobertura ausente. F1: mi propio fix de CR1 dejó un lector sin pelar. El chequeo de deriva online del doctor comparaba el OID crudo de ls-remote contra source.commit, ya pelado, así que reportaba 'source advanced or disappeared' sobre un repo sano con fuente-tag. Añadido remoteCommit, que toma la entrada pelada que ls-remote ya publica y no necesita fetch; verificado contra git que el patrón exacto NO lista la línea ^{} y hace falta el glob, filtrando luego por nombre exacto para no colar refs hermanas como refs/tags/v10. Certificado además que TODOS los lectores del valor registrado pelan igual: revalidatePlan, el doctor offline, activationRemovals y el online; fetchRef sigue crudo a propósito porque ahí lo que se clasifica es el objeto. F2: el aserto de 'ninguna rama de recovery' usaba un patrón sin glob que nunca casa, y aun con el glob seguía siendo vacuo porque el fixture no declara integration_branch y exige confirmed igual a observed, así que la rama no podía nacer. Movido a un caso propio con esos dos obstáculos quitados y con el aserto ANTES del throws, porque como assert.throws primero un guard ausente falla ahí y la cláusula nunca se evalúa: eso es lo que la mantuvo inverificable dos rondas. Certificado retirando el guard: sale refs/heads/changeledger/recover-<oid> y el aserto dispara. F3: restaurada la ruta de escape. abortStatePending lee sin clasificar solo en modo offline, porque descartar una ref es la reparación de la corrupción que todo lo demás rechaza y no adopta nada; el cálculo de staleness posterior tolera un confirmed corrupto para no convertir una reparación completada en un error. Documentado el remedio en README, que antes prometía una salida inalcanzable. Añadido CR10. F4 y F5: tareas del Plan resueltas y corregida la errata de 'cinco'. Matriz: control 1.163/1.163 sobre suite completa; M8 doctor crudo mata solo el test del doctor online; M9 abort clasificando mata solo CR10; M6 resolvedor compartido mata los dos tests de CR9. Sin colaterales.
- **2026-07-25T13:36:42Z** `[status]` in-progress → in-review
- **2026-07-25T13:55:49Z** `[review]` in-review → in-progress (retry): F1 bloqueante e introducido por mí en 337eeab3, en la función que el propio brief marcó para escrutinio: remoteCommit deriva la ausencia de si el GLOB casó algo, no de si casó el nombre exacto. Como el glob admite hermanas a propósito, una refs/tags/v1 desaparecida junto a una refs/tags/v10 existente produce salida no vacía sin línea exacta, así que oid queda undefined y lanza 'returned malformed Git output'. El bucle online del doctor no tiene try/catch, al contrario que su hermano offline, así que el comando entero revienta con un mensaje que culpa al formato de salida de git en vez de reportar 'source advanced or disappeared'. remoteRef devolvía null en ese caso, así que es regresión de esta vuelta. F2 menor: el párrafo nuevo del README afirma que el diagnóstico nombra la ref, y solo nombra el OID; eso solo es cierto en la ruta v1. Además queda la cláusula vacua de CR5, misma clase que la que acabo de arreglar en CR9.
- **2026-07-25T14:02:45Z** `[note]` Corrección del cuarto retry. F1 era mío y de la misma forma que me avisaron: introduje el glob en remoteCommit y no actualicé la comprobación de ausencia que el glob invalida. Como el glob admite hermanas por diseño, una fuente desaparecida junto a una ref con su nombre como prefijo producía salida no vacía sin línea exacta, oid quedaba undefined y lanzaba 'returned malformed Git output'; y como el bucle online del doctor no tiene try/catch —al contrario que su hermano offline— el comando entero reventaba culpando al formato de salida de git en vez de reportar la divergencia. La ausencia se deriva ahora del nombre exacto y el throw queda solo para una línea casada con OID malformado. Ampliado CR1 con ese caso y certificado por mutación: volver a keyear la ausencia en el glob mata solo ese test. F2: el README afirmaba que el diagnóstico nombra la ref y solo nombra el OID; corregido con el mensaje literal y con la indicación de comprobar las tres refs con git for-each-ref, porque la herramienta no dice cuál de ellas guarda el objeto malo. Retirada además la cláusula vacua de CR5, misma clase que la que arreglé en CR9: ese fixture no declara integration_branch, así que la rama no podía nacer ni sin guard y el aserto pasaba por la razón equivocada; queda solo el matcher del diagnóstico, que sí ancla, y la afirmación de 'nada escrito' vive en el caso de CR9 construido para ella. Gate 1.164/1.164 con skipped 0, Biome limpio, 246 changes válidos.
- **2026-07-25T14:02:53Z** `[status]` in-progress → in-review
- **2026-07-25T14:20:44Z** `[review]` in-review → in-validation (delegated subagent, clean context)
- **2026-07-25T14:20:44Z** `[note]` Quinta revisión limpia: pass. Verificado por ejecución sobre diez formas de remoto que la corrección de ausencia elige el OID correcto o devuelve null en cada una, incluido el caso ambiguo con v1, v1^{}, v10 y v10^{} anunciados a la vez; que remoteCommit tiene un único llamador y que null se diagnostica como desaparecida y nunca como sin cambios; y que el test nuevo carga peso, porque retirar la corrección deja exactamente un fallo. Gate 1.164/1.164 con skipped 0 y 246 changes válidos. Queda un hallazgo bajo sin corregir, y lo registro sin maquillarlo porque es una afirmación falsa mía en código entregado: al retirar la cláusula vacua de CR5 escribí que la afirmación de 'ninguna rama de recovery' queda anclada en el caso de CR9, y no es cierto. El Given de CR9 es un tip de réplica no-commit, guardado en readStateReplica; el de CR5 es una activation ref no-commit, guardada en activationCommitOid: sitio de corrupción distinto, guard distinto y módulo distinto. La cláusula de CR5 sigue en la especificación y hoy no la asevera ningún test. El revisor demostró que sí es anclable con un fixture viable de cuatro líneas —declarando integration_branch y moviendo el aserto antes del throws— y que sin el guard nace refs/heads/changeledger/recover-<oid>, que es precisamente AUTH-12. El comportamiento entregado es correcto y el guard está anclado por otros tres tests; lo que falta es el ancla de regresión de la mitad de efecto secundario. Se traslada al humano en validación en lugar de editar después de un pass.
- **2026-07-25T14:24:57Z** `[validation]` in-validation → in-progress (human rejected via conversation): El humano ordena aplicar la corrección del hallazgo bajo de la quinta revisión: anclar la cláusula de CR5 con un fixture viable y retirar la afirmación falsa de que el caso de CR9 la ancla.
- **2026-07-25T14:30:42Z** `[note]` Corrección ordenada por el humano tras el pass. Anclada la cláusula de CR5 en un fixture viable: declara git.integration_branch, sin lo cual recovery rehúsa antes de llegar al guard y asertar la ausencia de rama pasaría por la razón equivocada, y el aserto de la rama va ANTES del throws porque como assert.throws primero un guard ausente falla ahí y la cláusula no se evalúa. Retirada la afirmación falsa que yo había escrito al quitar la cláusula vacua: el caso de CR9 NO ancla el de CR5, porque su corrupción es un tip de réplica guardado en readStateReplica mientras el de CR5 es una activation ref guardada en activationCommitOid; sitio, guard y módulo distintos. Certificado retirando la clasificación de activationCommitOid: nace refs/heads/changeledger/recover-<oid> y dispara ese aserto y no otro. Añadido además el caso presente-con-hermana que el revisor midió sin contarlo como defecto, porque con las dos tags anotadas el remoto anuncia v1, v1^{} , v10 y v10^{} y una simplificación a lines[0] observaría el commit de la hermana; certificado que esa simplificación mata los tres tests de CR1. Se declara como decisión propia haber sumado ese caso a la corrección pedida: son tres líneas en un fichero que ya estaba abierto y evitaba una ronda entera después. Gate 1.165/1.165 con skipped 0, Biome limpio, 246 changes válidos. Sin commitear por la regla 7, al ser corrección de un error reportado por el humano.
- **2026-07-25T14:30:42Z** `[status]` in-progress → in-review
- **2026-07-25T14:38:46Z** `[review]` in-review → in-validation (delegated subagent, clean context)
- **2026-07-25T14:38:46Z** `[note]` Sexta revisión limpia: pass sin hallazgos, con alcance verificado a tres ficheros y nada bajo src, bin ni hooks. Certificado por ejecución que la cláusula de CR5 carga peso —retirar la clasificación de activationCommitOid materializa refs/heads/changeledger/recover-<oid> y dispara ese aserto, no el del diagnóstico, y el fixture llega hasta createBranchCommit, así que no pasa por una razón nueva equivocada— y que el caso presente-con-hermana discrimina dos commits distintos, con el anuncio de cuatro líneas y cuatro OIDs distintos volcado para probarlo, muriendo bajo lines[0] y bajo un prefix-match. Gate 1.165/1.165 con skipped 0, Biome limpio y 246 changes válidos. Observación registrada por el revisor y no contada como defecto: CR1 ganó texto de criterio para el caso desaparecida-con-hermana en la ronda anterior, pero el caso presente-con-hermana no tiene cláusula propia; la dirección del contrato es criterio hacia test, así que un test más allá de lo escrito no es incumplimiento, y la adición queda declarada en el Log. No se corrige después de un pass. Pendiente de aceptación humana; por la regla 7 la corrección sigue sin commitear.
- **2026-07-25T14:43:33Z** `[validation]` in-validation → done (human accepted)
- **2026-07-25T14:44:24Z** `[note]` Corrección commiteada por instrucción del humano antes de su aceptación, para no dejar el trabajo solo en el working tree al pausar. Es una desviación consciente de la regla 7 de INTENT, que la mantendría sin commitear hasta la aceptación final; queda registrada aquí en lugar de aplicarse en silencio. El change sigue en in-validation y la aceptación sigue pendiente.
